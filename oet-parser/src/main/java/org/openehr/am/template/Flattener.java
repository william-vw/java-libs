 ...
 * OET-template parser and join constraints from the parent archetype and all
 * included children archetypes, and output the combined constraints in pure AOM
 * instance.
 * 
 * @author rong.chen
 *
...
public class Flattener {
	 ...
	 * @throws Exception
	 */
	public Archetype toFlattenedArchetype(TEMPLATE template, Map<String, Archetype> archetypeMap)
			throws FlatteningException {

		return toFlattenedArchetype(template, archetypeMap, new HashMap<String, TEMPLATE>());
	}
	...
	public Archetype toFlattenedArchetype(TEMPLATE template, Map<String, Archetype> archetypeMap,
		...
		log.debug("Loaded archetype/template maps, total archetypes: " + archetypeMap.size() + ", total templates: "
	...
	private Archetype flattenItem(Archetype parentArchetype, ITEM item) throws FlatteningException {
		...
		fillArchetypeSlot(parentArchetype, archetype, item.getPath(), item.getName());
	...
	private Archetype flattenComposition(COMPOSITION composition) throws FlatteningException {
		...
		if (items != null && items.length > 0) {
			...
			if (contentAttribute == null) {
				...
				contentAttribute = new CMultipleAttribute(path, CONTENT, Existence.OPTIONAL, Cardinality.LIST,
						alternatives);
		...
		applyNameConstraint(archetype, archetype.getDefinition(), composition.getName(), "/");
	...
	private Archetype flattenContentItem(Archetype parentArchetype, ContentItem definition) throws FlatteningException {
		...
		log.debug("flattening content_item on archetype: " + definition.getArchetypeId() + " on path: "
				+ definition.getPath());
	...
	private void updatePathWithNamedNodeOnCObjectTree(CObject cobj, String nodeId, String name)
			throws FlatteningException {

		// TODO shouldn't happen
		if (cobj == null) {
			log.warn("null cobj in updatePathWithNamedNodeOnCObjectTree(): " + nodeId + "/" + name);
			return;
		}
	...
	private String replaceNodeIdWithNamedNode(String path, String nodeId, String name) {
		return path.replaceFirst("\\[" + nodeId + "\\]", "\\[" + namedNodeSegment(nodeId, name) + "\\]");
	}
	...
	private Archetype flattenSection(Archetype parentArchetype, Archetype archetype, SECTION section)
			throws FlatteningException {
		...
		if (items != null && items.length > 0) {
			...
			if (itemsAttribute == null) {
				...
				List<CObject> alternatives = new ArrayList<CObject>();
				itemsAttribute = new CMultipleAttribute(path, ITEMS, Existence.OPTIONAL, Cardinality.LIST,
						alternatives);
		...
		for (CObject cobj : parent.getChildren()) {
			if (nodeId.equals(cobj.path()) && name.equals(cobj.getNodeId())) {
				throw new FlatteningException(
						"duplicated node_id/name: " + nodeId + "/" + name + "at path: " + parent.path());
			}
		}
	}
	...
	/**
	 * Overwrite recursively all the nodeIds in the given cobj tree with incremental
	 * number starting with given count value
	 * 
	 * @param ccobj
	 * @param diff
	...
	private Archetype flattenEntry(Archetype parentArchetype, Archetype archetype, ENTRY definition)
			throws FlatteningException {

		log.debug("flattening entry on archetype: " + definition.getArchetypeId() + ", path: "
				+ ((ENTRY) definition).getPath());
	...
	/**
	 * Fill archetype_slot at specified path of parentArchetype and remove all slots
	 * 
	 ...
	 * @throws FlatteningException
	 */
	private void fillArchetypeSlot(Archetype parentArchetype, Archetype archetype, String path, String name)
			throws FlatteningException {
		...
		if (parentArchetype != null) {
			...
			setPathPrefixOnCObjectTree(root, attribute.path() + "[" + pathSegment + "]");
	...
	private void flattenInstruction(Archetype parentArchetype, Archetype archetype, INSTRUCTION instruction)
			throws FlatteningException {

		log.debug("flattening instruction on archetype: " + instruction.getArchetypeId() + ", path: "
				+ ((ENTRY) instruction).getPath());
	...
	private void removeArchetypeSlots(CAttribute cattr) {
		...
		for (Iterator<CObject> it = cattr.getChildren().iterator(); it.hasNext();) {
			...
			if (cobj instanceof ArchetypeSlot) {
				it.remove();
				log.debug("archetype_slot removed from attribute " + cattr.getRmAttributeName());
			}
		}
	}
	 ...
	 * @throws FlatteningException
	 */
	private Archetype flattenItemStructure(Archetype parentArchetype, ITEMSTRUCTURE structure)
			throws FlatteningException {

		log.debug("flattening item_structure on archetype: " + structure.getArchetypeId() + " on path: "
				+ structure.getPath());
		...
		fillArchetypeSlot(parentArchetype, archetype, structure.getPath(), structure.getName());
	 ...
	 * 
	 * Important to keep passing the same archetype reference and keep the archetype
	 * updated when each rule is applied
	 * 
	 * @param archetype
	 * @param rules
	 * @throws FlatteningException
		...
		for (Statement rule : rules) {
			if (rule.getName() != null) {
				if (name == null) {
					...
					log.debug("more than one named node [" + name + "] on path: " + leadingPath);
	...
	void applyRule(Archetype archetype, Statement rule) throws FlatteningException {
		...
		log.debug("apply rule [" + rule + "] on archetype: " + archetype.getArchetypeId().toString());
		...
		constraint = applyNameConstraint(archetype, constraint, rule.getName(), path);
	 ...
	 * 
	 * Example: <Rule path="/items[at0001]" default="Ipren" />
	 ...
	 */
	protected void applyDefaultValueConstraint(ArchetypeConstraint constraint, String defaultValue)
			throws FlatteningException {
		...
		if (!(constraint instanceof CComplexObject)) {
			...
			throw new FlatteningException(
					"failed to apply default constraint," + "unexpected constraint node: " + constraint.getClass());
		}
		...
		if (!ccobj.getRmTypeName().equalsIgnoreCase(ELEMENT)) {
			...
			throw new FlatteningException("failed to apply default constraint," + "unexpected rmType["
					+ ccobj.getRmTypeName() + "] on node: " + ccobj.path());
		}
		...
		if (cattr == null) {
			cattr = new CSingleAttribute(ccobj.path() + "/" + VALUE, VALUE, Existence.REQUIRED);
		...
		if (defaultValue.indexOf("::") < 0) {
			...
			CodePhrase codePhrase = parseCodePhraseAndCollectText(defaultValue, path);
	...
	protected void applyOccurrencesConstraints(Archetype archetype, ArchetypeConstraint constraint, Statement rule)
			throws FlatteningException {
		...
		if (max != null || min != null) {
			if (constraint instanceof CObject) {
				applyOccurrencesConstraint(archetype, (CObject) constraint, rule.getMax(), rule.getMin());
			}
		}
	}

	protected void applyOccurrencesConstraint(Archetype archetype, CObject cobj, BigInteger max, BigInteger min)
			throws FlatteningException {

		log.debug("applyOccurrencesConstraint, min: " + min + ", max: " + max + ", at: " + cobj.path());
		...
		if ((!"/".equals(path)) && occurrences == null) {
			...
			log.warn("try to set occurrences constraint on default(null)" + " occurrences: " + cobj.path());
			return;
		}

		if (max != null && occurrences.getUpper() != null && max.intValue() > occurrences.getUpper().intValue()) {
			throw new FlatteningException("more permissive max occurrences, " + path);
		}

		if (max != null && occurrences.getLower() != null && max.intValue() < occurrences.getLower().intValue()) {
			throw new FlatteningException("contradicting max occurrences [max: " + max.intValue()
					+ ", occurrences.lower: " + occurrences.getLower().intValue() + "] at path: " + path);
		}

		if (min != null && occurrences.getLower() != null && min.intValue() < occurrences.getLower().intValue()) {
			throw new FlatteningException("more permissive min occurrences, " + path);
		}

		if (min != null && occurrences.getUpper() != null && min.intValue() > occurrences.getUpper().intValue()) {
		...
		if ((!"/".equals(path)) && occurrences.getUpper() != null && occurrences.getLower() != null
				&& occurrences.getUpper().intValue() == 1 && occurrences.getLower().intValue() == 1) {
			log.warn("try to set occurrences constraint on required node: " + cobj.path());
			return;
		}
		...
		Interval<Integer> newOccurrences = new Interval<Integer>(lower, upper, lower != null, upper != null);

		log.debug("newOccurrences: " + newOccurrences);

		if (newOccurrences.getLower() != null && newOccurrences.getLower().intValue() > 0 && !"/".equals(path)) {
	...
	protected CComplexObject applyNameConstraint(Archetype archetype, ArchetypeConstraint constraint, String name,
			String localPath) throws FlatteningException {

		log.debug("applying name constraint [" + name + "] on path: " + localPath + "constraint: " + constraint.path());
	...
	private void removeUnnamedSiblingNode(Archetype archetype, CAttribute parent, String nodeId)
			throws FlatteningException {
		...
		for (Iterator<CObject> it = parent.getChildren().iterator(); it.hasNext();) {
			...
			if (nodeId.equals(cobj.getNodeId())) {
				if (!(cobj instanceof CComplexObject)) {
					throw new FlatteningException("unexpected constraint type: " + cobj.getClass() + " for node_id["
							+ nodeId + "] at " + parent.path());
				}
				CComplexObject ccobj = (CComplexObject) cobj;
	 ...
	 * 
	 * Example: "/data[at0001]/items[at0002 and name/value='one']/items[at0003]"
	 */
...
//				}
	...
	protected void applyTextConstraint(CComplexObject ccobj, TextConstraint tc) throws FlatteningException {
		...
		if (includedValues != null && includedValues.length > 0) {
			...
			CComplexObject valueObj = CComplexObject.createSingleRequired(valuePath, DV_CODED_TEXT);
			...
			CCodePhrase ccp = CCodePhrase.singleRequired(definingCodePath, terminology, codeList);
			definingCode.addChild(ccp);

		} else if (excludedValues != null && excludedValues.length > 0) {
			if (valueAttr == null) {
				throw new FlatteningException(
						"failed to set excluded values on text constraint, " + "VALUE attribute missing");
			}
			if (valueAttr.getChildren().size() == 1) {
				CObject child = valueAttr.getChildren().get(0);
				if (child instanceof CComplexObject) {
					...
					throw new FlatteningException(
							"Unexpected VALUE.child constrainType: " + (child == null ? "null" : child.getClass()));
				}
			} else {
				throw new FlatteningException("Unexpected VALUE.children.size: " + valueAttr.getChildren().size());
			}
		}
	}
		...
		if (cattr != null) {
			...
			for (CObject child : cattr.getChildren()) {
				...
				for (String type : includedTypes) {
					if ((DATA_TYPES_PREFIX + type).equalsIgnoreCase(childType) || type.equalsIgnoreCase(childType)) {
			...
			for (String rmType : includedTypes) {
				...
				CComplexObject child = CComplexObject.createSingleRequired(valuePath, rmType);
				cattr.addChild(child);
			}
			log.debug("value attribute added with total " + cattr.getChildren().size() + "child(s)");
		}
	}
	...
	/*
	 * Parse coded text like 'SNOMED-CT::258835005::mg/dygn' the last segment of
	 * actual text value is now collected in termMap as a quick fix
	 * 
	 */
